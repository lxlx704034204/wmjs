...
(function (window, $) {
    ...
    var sjPager = window.sjPager = {
        ...
        commonHtmlText: {
            ...
            firstResultHtml: "<a href='javascript:void(0)' onclick='sjPager.doPage({0},{1},{2})'>{3}</a>",
        ...
        init: function (obj,op) {
            ...
            _self.doPage(_self.opts.firstResult, _self.opts.maxResults, _self.opts.searchParam);

            return _self.opts;
        },
        doPage: function (index, maxResults, searchParam) {
            var _self = this;

            _self.opts.firstResult = index;
            _self.opts.maxResults = maxResults;
            _self.opts.searchParam = searchParam;

            $.ajax({
                type: _self.opts.type,
                data: $.extend(_self.opts.searchParam || {}, {
                    firstResult: (_self.opts.firstResult-1)*_self.opts.maxResults,
                    maxResults: _self.opts.maxResults || 10
                }),
                ...
                success: function (data) {
                    ...
                    _self.opts.totalCount = data.length>0 ? data[0].countResults:0;
                    _self.getTotalPage();
                    if (_self.opts.totalCount > 0 && _self.opts.firstResult > 0) {
        ...
        getTotalPage: function() {
            ...
            _self.opts.totalPage = Math.ceil(_self.opts.totalCount / _self.opts.maxResults);
        },
        createPreAndFirstBtn: function (pageTextArr) {
            var _self = this;

            if (_self.opts.firstResult == 1) {
                ...
                pageTextArr.push(_self.createIndexText(_self.opts.firstResult - 1, _self.opts.preText));
            }
        },
        createNextAndLastBtn: function (pageTextArr) {
            var _self = this;
            if (_self.opts.firstResult == _self.opts.totalPage) {
                ...
                pageTextArr.push(_self.createIndexText(_self.opts.firstResult + 1, _self.opts.nextText));
                if (_self.opts.showLast)
        ...
        createIndexBtn: function (pageTextArr) {
            ...
            */
            ...
            /*页码*/
            if (_self.opts.firstResult > shiftingLeftStart) {
                ...
                for (i = _self.opts.firstResult - _self.opts.shiftingLeft; i < _self.opts.firstResult; i++) {
                    pageTextArr.push(_self.createIndexText(i, i));
                }

            } else {
                for (i = 1; i < _self.opts.firstResult; i++) {
                    pageTextArr.push(_self.createIndexText(i, i));
                }
            }

            pageTextArr.push(_self.createSpan(_self.opts.firstResult, 'current'));

            if (_self.opts.firstResult <= shiftingRightStart) {

                for (i = _self.opts.firstResult + 1; i < _self.opts.firstResult + 1 + _self.opts.shiftingRight; i++) {
                    pageTextArr.push(_self.createIndexText(i, i));
                }
                ...
                for (i = _self.opts.firstResult + 1; i <= _self.opts.totalPage; i++) {
                    pageTextArr.push(_self.createIndexText(i, i));
                }
            }
        },
        renderHtml: function (pageTextArr) {
            ...
            _self.pagerElement.html(pageText).append(stringFormat(_self.commonHtmlText.rightHtml, _self.opts.totalPage, _self.opts.firstResult)).append(_self.commonHtmlText.clearFloatHtml);
        },
        ...
        createIndexText: function (index, text) {
            var _self = this;
            return stringFormat(_self.commonHtmlText.firstResultHtml, index, _self.opts.maxResults, JSON.stringify(_self.opts.searchParam), text);
        },
        jumpToPage: function() {
            var _self = this;
            ...
            if (!isNaN(index) && index > 0 && index <= _self.opts.totalPage) {
                _self.doPage(index, _self.opts.maxResults, _self.opts.searchParam);
            } else {
                $txtToPager.focus();
            }
        }
